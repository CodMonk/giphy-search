      ...
      if (searchText) {
        ...
        if (resp && resp.data) {
          const imageListData: ImageList = {
            data: resp.data.data.map((item: ImageType) => ({ id: item.id, url: item.images.fixed_height_small.url })),
          };
  ...
  return (
    <div>
      ...
      <div id="imageContainer" className="image-container">
        {imageList && imageList.data.length
          ? imageList.data.map((image, index) => {
              return <img key={index} className="image" src={image.url}></img>;
            })
          : imageList === null
          ? ''
          : 'No image available for entered text'}
      </div>
    </div>
  );
};
